...
This guide covers the authorization system of the TYPO3 Monitoring,
including built-in authorizers and how to create custom authorization
strategies.
...
- Check that the authorizer is properly registered
...
#### IP Authorization Problems
- Check for load balancer headers
- Verify CIDR notation syntax
- Test with actual client IPs

### Debug Authorization

Add logging to your authorizers:

```php
public function isAuthorized(ServerRequestInterface $request): bool
{
    $result = $this->performAuthorization($request);
